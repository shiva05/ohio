...
export class DownloadPDFService {
  ...
  public asSaveToProfile(obj): Observable<any> {
    const uri = environment.AsSaveToProfile + '?moduleKey=203&detailKey=0&assetTemplateKey=22203&docType=3';
    // this.http refers to HttpClient. Note here that you cannot use the generic get<Blob> as it does not compile: instead you "choose" the appropriate API in this way.
    return this.httpService.post(uri, obj);
  }
  public csSaveToProfile(obj): Observable<any> {
    const uri = environment.CsSaveToProfile + '?moduleKey=203&detailKey=0&assetTemplateKey=22203&docType=3';
    // this.http refers to HttpClient. Note here that you cannot use the generic get<Blob> as it does not compile: instead you "choose" the appropriate API in this way.
    return this.httpService.post(uri, obj);
  }
}
